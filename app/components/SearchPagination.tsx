...
export default function SearchPagination({ pagesCount, currentPage }: SearchPaginationProps) {
	...
	return (
		<div className="mb-4 mt-auto flex w-full items-center justify-center gap-1 lg:gap-3">
			<Link
				key={"pageBefore"}
				href={`${hrefPage}&page=${currentPage - 1 > 0 ? currentPage - 1 : 1}`}
			>
				<button
			...
			{paginationRange.map((page) => (
				<Link key={page} href={`${hrefPage}&page=${page}`}>
					<button
			...
			<Link
				key={"pageAfter"}
				href={`${hrefPage}&page=${currentPage + 1 <= pagesCount ? currentPage + 1 : pagesCount}`}
			>
				<button
